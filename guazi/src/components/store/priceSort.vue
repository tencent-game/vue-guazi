<template>
    <div class="sortAll">
        ...
        <ul class="price_sort">
            <li v-for="(sort,index) in priceSort" :key="sort" @click="clickIt(index)" :class="{active:index===currentIndex}" >{{sort}}</li>
        </ul>
...
// import sortCarlist from '../../components/store/sortCarlist'
import Vue from 'vue'
...
export default {
    data(){
        return {
            currentIndex:0,
            priceSort:["全部","0-5万","5-10万","10-15万","15万以上"],
            loading: false,
            finished: false,
            carList:[],
            carLists:[],
    ...
    methods:{
        ...
        async loadData(){
            // this.page++
            let result=await http.get('/aaa/clientUc/post/postList',{domain:this.domain,type:5,num:20,platform:2,page:this.page,parkingLotId:826,user_city_id:this.user_city_id})
            console.log(result)
            !this.carList?this.carList.push(...this.carList).push(...result.data):this.carList.push(...result.data)
            
            this.loading = false
            this.page++
            if(this.carList.length>150){
                this.loading=true
                this.finished=true
            }
            
            setTimeout(()=>{this.switchone(),0} )
            ...
            //         // this.carLists=[]
            //         this.carLists=this.carList.filter((item)=>{item.attributes.price<=50000})
            //         return
            //     case (this.currentIndex===2):
            //         this.carLists=this.carList.filter((item)=>{item.attributes.price>=50000&&item.attributes.price<=100000})
            //         return
            //     case (this.currentIndex===3):
            //         this.carLists=this.carList.filter((item)=>{item.attributes.price>=100000&&item.attributes.price<=150000})
         ...
         switchone(){
             switch(this.currentIndex){
                ...
                case 1:
                    // this.carLists=[]
                    this.carLists= _.filter(this.carList, o => {
                            return o.attributes.price<=50000
                        })
                        return this.carLists
                    
                case 2:
                    this.carLists= _.filter(this.carList, o => {
                            return o.attributes.price>=50000 && o.attributes.price<=100000
                        })
                        return this.carLists
                case 3:
                    this.carLists= _.filter(this.carList, o => {
                            return o.attributes.price<=150000 && o.attributes.price>=100000
                        })
                        return this.carLists
                case 4:
                    this.carLists=_.filter(this.carList,o=>{
                        return o.attributes.price>150000
                    })
                    return this.carLists
            }
        },
...
<style lang="stylus" scoped>
    ...
    .sortAll
        ...
        .price_sort
            ...
            z-index 999
            box-shadow: 0 0.01rem 0.03rem 0 #e7ecf4;
            li
                flex 1
                height 100%
                text-align center
